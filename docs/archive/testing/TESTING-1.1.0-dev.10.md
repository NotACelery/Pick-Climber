# Pick Climber 1.1.0-dev.10 — Integration repair + options GUI QA

## Build / startup

- [ ] `compileJava` no longer reports a `Consumer<Double>` / `DoubleConsumer` mismatch in `PickClimberOptionsScreen`.
- [ ] `build.bat` completes with Java 21.
- [ ] The exact dev.10 JAR starts in NeoForge 1.21.1 without client startup errors.
